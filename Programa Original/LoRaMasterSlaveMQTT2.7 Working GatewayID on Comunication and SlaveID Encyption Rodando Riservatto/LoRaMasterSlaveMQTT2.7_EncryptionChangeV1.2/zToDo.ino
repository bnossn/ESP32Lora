...
(Input APP) Resetar os bool dos outputs no receive e seta-los quando receber ordem do usuario atraves do app. ok
  ...
  Retirar o msgBegin do protocolo de envio de mensagem ok
  Gateway só manda as IO's que interessam. Fazer um setup via websocket Nok
  Implementar Redis Nok
  ...
  Implementar filtro analogico ok
  Implementar Comandos WebSocket Slave. ok
  Fazer o Slave receber o ID do master como Persistente ok
  Implemtar ListMinVar via Websocket ok
  Receber variaveis de conversao dos AI`s como persistente. ok
  Tranformar váriaveis de interesse em persistente. Quando reiniciado. Puxar status anterior da memoria? ok
  Fazer Master comunicar com rpi via serial. ok
  Fazer o Slave usar o ID do Master durante a comunicação. ok
  Gateway fica com o set-up dos sensores do Slave (Conversão para uma unidade conhecida) (Colocar no Slave) ok 


  Atualizar o status dos IO`s como persistente?
