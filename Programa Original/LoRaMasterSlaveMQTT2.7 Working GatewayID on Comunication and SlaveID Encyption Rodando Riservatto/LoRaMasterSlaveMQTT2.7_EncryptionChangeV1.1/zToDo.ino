...
(Input APP) Resetar os bool dos outputs no receive e seta-los quando receber ordem do usuario atraves do app. ok
  ...
  Retirar o msgBegin do protocolo de envio de mensagem
  Gateway só manda as IO's que interessam. Fazer um setup via websocket
  Gateway fica com o set-up dos sensores do Slave (Conversão para uma unidade conhecida) (Colocar no Slave)
  ...
  Fazer Master comunicar com rpi via serial.
  Atualizar o status dos IO`s como persistente?
  Fazer o Slave usar o ID do Master durante a comunicação.
