  ...
  if (slaves[0].id != 0){ //Só comunica se houver algum Slave registrado
    ...
    while (!messageSent){
      ...
      if ((millis() - lastSentTime > INTERVAL) || (firstMsg == true)) {
        ...
        //send(nextSlaveIndex);
        SendGTWmsg("longMessageTesting", getChipID(), nextSlaveIndex, false);
...
void send(int slaveIndex) {
 ...
 // !!!! Create Header
  Header header;
  strcpy(header.msgBegin, GETDATA.c_str()); //"g="
 ...
 // !!!! Create gtwData
  struct gtwData *gtwDataPtr = (struct gtwData*) malloc(sizeof(struct gtwData));

  strcpy(gtwDataPtr->gtwMsg, data.c_str());
  gtwDataPtr->idDestino = slaves[slaveIndex].id;
  
  //FIX ME - DATA BELOW HAS TO BE SET SOMEWHERE
  gtwDataPtr->imsgCounter = 0;
  gtwDataPtr->rstCounter = false;
 // !!!! \Create gtwData

  //Encrypt Gateway Data
  gtwDataPtr = (struct gtwData*) encryptData(gtwDataPtr, sizeof(struct gtwData), aes_key, header.AES_iv);
  ...
  LoRa.write((uint8_t *) gtwDataPtr, sizeof(struct gtwData));
...
void SendGTWmsg(char data[], uint64_t idOrigem, uint16_t slaveIndex, boolean rstCounter) {

  char msgToEncrypt[255];
  const char* token = "&";
  ...
  //IDdestino&1/0&Counter&MSG
  strcpy(msgToEncrypt, data); //Data de interesse
  strcat(msgToEncrypt, token); //&
  sprintf (Sbuffer, "%i", slaves[slaveIndex].imsgCounter); //Counter
  strcat(msgToEncrypt, Sbuffer); //Counter
  strcat(msgToEncrypt, token); //&
  sprintf (Sbuffer, "%i", rstCounter); // reset counter
  strcat(msgToEncrypt, Sbuffer);  // reset counter
  strcat(msgToEncrypt, token); //&
  strcat(msgToEncrypt, uint64ToHexStr(slaves[slaveIndex].id).c_str()); //idDestino

  Header header;

  strcpy(header.msgBegin, GETDATA.c_str()); //"g="
  header.idOrigem = idOrigem;

  memcpy(header.AES_iv, aes_iv, sizeof(aes_iv));
  //sprintf(cleartext, msgToEncrypt);
  //sprintf(ciphertext, "%s", encrypt(msgToEncrypt, header.AES_iv));

  header.iCipherSize = strlen(ciphertext);

  if (bEncryptDebug) {
    Serial_Print(bDebug, "Encrypted Result: ");
    Serial_Println(bDebug, ciphertext);
    Serial_Println(bDebug, "\n" );
  }
