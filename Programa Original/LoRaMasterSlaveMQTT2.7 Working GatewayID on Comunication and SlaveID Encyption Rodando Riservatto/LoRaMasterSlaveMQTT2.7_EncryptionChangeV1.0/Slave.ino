...
void receiveV2(uint64_t idOrigem) { //gateway
  int packetSize = LoRa.parsePacket();

  if (packetSize) { //recebeu algo
    Serial.printf("onReceive(%i)\n", packetSize);

    Header header;
    LoRa.readBytes((uint8_t *)&header, sizeof(header)); //Le o Struct

    Serial_Print(bDebug, "idOrigem: ");
    Serial_Println(bDebug, uint64ToHexStr(header.idOrigem).c_str());
    Serial_Print(bDebug, "AES_iv[16]: ");
    print_key_iv(header.AES_iv);
    Serial_Print(bDebug, "iCipherSize: ");
    sprintf (Sbuffer, "%i", header.iCipherSize);
    Serial_Println(bDebug, Sbuffer);
    Serial_Print(bDebug, "msgBegin: ");
    Serial_Println(bDebug, header.msgBegin);
    

    if (header.idOrigem == idOrigem) { //Check se mensagem veio do device esperado
      Serial_Println(bDebug, "Passed the idOrigem Check: ");

      //char cipherRec[255];
      String cipherRec;
      //Armazena os dados do cipher em uma string
      while (LoRa.available()) {
        //strcat(cipherRec, (const char*) LoRa.read());
        cipherRec += (char) LoRa.read();
      }

      Serial_Print(bDebug, "Received: ");
      Serial_Println(bDebug, cipherRec.c_str());

      if (header.iCipherSize == strlen(cipherRec.c_str())) { //Check se mensagem tem tamanho esperado
        Serial_Println(bDebug, "Passed the iCipherSize Check: ");
        
        String decrypted = decrypt((char*) cipherRec.c_str(), header.AES_iv);
        if (bEncryptDebug) {
        ...
        //Continuar com o break down do cipher
        /*int i=0;
        char *idToken;
        idToken = strtok((char*) decrypted.c_str(), "&");
        while (idToken != NULL){
          slaves[i].id = strtoull(idToken, NULL, 16);
          idToken = strtok (NULL, "-");
          i++;
          if (i > NMAX_SLAVES-1) i = NMAX_SLAVES-1;
