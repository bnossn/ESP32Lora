...
#define INPUTUPDT_INTERVAL 500
long lastInputUpdt = 0;
...
// Conta quantos possiveis erros de comunicação (Testa estabilidade)
int comErrorCounter = 0;
int comReceivedCounter = 0;
...
void setup() {
  ...
  sprintf(Sbuffer,"sizeof(Header): %i \n", sizeof(struct Header));
  Serial_Println(bDebug, Sbuffer);
  sprintf(Sbuffer,"sizeof(gtwData): %i \n", sizeof(struct gtwData));
  Serial_Println(bDebug, Sbuffer);
  sprintf(Sbuffer,"sizeof(slaveData): %i \n", sizeof(struct slaveData));
  Serial_Println(bDebug, Sbuffer);
...
void loop() {
  ...
  if (millis() - lastInputUpdt > INPUTUPDT_INTERVAL) {
    //Faz update periodico dos AO`s
    lastInputUpdt = millis();
    updateInputVar();
    displayMessage(40);
  }
  ...
  if (millis() - slave.lastRecTime > COMMAXTIME) {
    ...
    if (!slave.isOnline) slave.isOnline = true;
  }
...
void IRAM_ATTR onReceive(int packetSize) {
 //receive(gatewayID, packetSize);
  ...
  if (packetSize >= (sizeof(struct Header) + sizeof(struct gtwData))) { //recebeu algo
    ...
    if (headerPtr->idOrigem == gatewayID) { //Check se mensagem veio do device esperado
      ...
      if (clearGTWDataPtr->idDestino == slave.id){
        ...
        if ((clearGTWDataPtr->imsgCounter > slave.imsgCounter) || (clearGTWDataPtr->bRstCounter == true)){ //Confere se mensagem nao e repetida ou se deve zerar contador
          ...
          Serial.println("PASSOU CLEARGTWDATAPTR");
  
          //updateInputVar();

          Serial.println("PASSOU updateInputVar");

          slave.packetRssi = LoRa.packetRssi();
          Serial.println("PASSOU packetRssi");
          slave.packetSnr = LoRa.packetSnr();
          Serial.println("PASSOU packetSnr");
          
          displayMessage(40);

          Serial.println("ANTES SEND");
  
          send();

          Serial.println("APOS SEND");
...
void receive(uint64_t idOrigem, int packetSize) {
...
void updateInputVar() {
  ...
  Serial.println("ANTES DIGITAL INPUT");
  ...
  for (int i = 0; i < nMaxDI; i++) {
    if (pinDigIn[i] >= 0) {
      ...
      Serial.println("DIGITAL INPUT:" + String(i, DEC));
  ...
  Serial.println("APOS DIGITAL INPUT");
...
void updateAnalogInput() {
  ...
  Serial.println("ANTES ANALOG INPUT");
  
  //Atualiza slave AI variables
  for (int i = 0; i < nMaxAI; i++) {
    if (pinAnIn[i] >= 0) {

      Serial.println("ANALOG INPUT:" + String(i, DEC));
      yield();
  ...
  Serial.println("APOS ANALOG INPUT");
