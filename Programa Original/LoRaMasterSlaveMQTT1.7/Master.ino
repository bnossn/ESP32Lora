...
void setup() {
  ...
  //Inicia Slaves com os ID`s indicados em compile time por "slavesID[]"
/*  for (int i = 0; i < (sizeof(slavesID) / sizeof(slavesID[0])); i++) {
    slaves[i].id = slavesID[i];
  }*/
...
void loop() {
  ...
  if (slaves[0].id != 0){ //Só comunica se houver algum Slave registrado
    ...
    //FIX ME: o intervalo de envio tem que ser independente do lastSentTime de cada slave (10s entre envios)
...
// !!!! <Atualiza Slave`s Variables>
  if (topic_ClintID.equalsIgnoreCase(String(clientID))){//Double Check ClientID
...
/*FIX ME: HAS TO USE WIFI EVENTS TO HANDLE DISCONNECTIONS.
