  ...
  if (slaves[0].id != 0){ //Só comunica se houver algum Slave registrado
    ...
    while (!messageSent){
      ...
      if ((millis() - lastSentTime > INTERVAL) || (firstMsg == true)) {
        ...
        send(nextSlaveIndex);
        //SendGTWmsg("DO0;AO00;AO10;", getChipID(), nextSlaveIndex, false);
...
void send(int slaveIndex) {
 ...
 // !!!! Create gtwData
  ...
  ClearGTWDataPtr = encodeSendData(slaveIndex);
 ...
 // !!!! \Create gtwData
  ...
  if (!slaves[slaveIndex].bmodSent && !ClearGTWDataPtr->bEmptyMsg){
    slaves[slaveIndex].bmodSent=true; //Se modificacao nao foi enviada, transforma-la em enviada e enviar.
  }
    ...
    if (headerPtr->idOrigem == slaves[slaveIndex].id) {//Check se mensagem veio do device esperado
      ...
      if (clearSlvDataPtr->idDestino == thisdev_ID){
        ...
        if (clearSlvDataPtr->imsgCounter == slaves[slaveIndex].imsgCounter){//Confere se msg recebida tem msm valor da msg enviada
          ...
          sprintf (Sbuffer, "slaves[slaveIndex].imsgCounter: %i", slaves[slaveIndex].imsgCounter);
          Serial_Println(bDebug, Sbuffer);
          sprintf (Sbuffer, "slaves[slaveIndex].DigIn: %X", slaves[slaveIndex].DigIn);
          Serial_Println(bDebug, Sbuffer);
          sprintf (Sbuffer, "slaves[slaveIndex].DigOut: %X", slaves[slaveIndex].DigOut);
          Serial_Println(bDebug, Sbuffer);
          sprintf (Sbuffer, "slaves[slaveIndex].AI[0]: %i", slaves[slaveIndex].AI[0]);
          Serial_Println(bDebug, Sbuffer);
          sprintf (Sbuffer, "slaves[slaveIndex].AI[1]: %i", slaves[slaveIndex].AI[1]);
          Serial_Println(bDebug, Sbuffer);
          sprintf (Sbuffer, "slaves[slaveIndex].AI[2]: %i \n\n", slaves[slaveIndex].AI[2]);
          Serial_Println(bDebug, Sbuffer);
...
/* Encode Msg de envio - iSetDO, iSetAO do gtwData
 */
struct gtwData* encodeSendData(int slaveIndex){
  struct gtwData *gtwEncodePtr  = (struct gtwData*) malloc(sizeof(struct gtwData));
  ...
  if (!(slaves[slaveIndex].DigOutmodified || 
    ...
    gtwEncodePtr->bEmptyMsg = true;
  }else{
    gtwEncodePtr->bEmptyMsg = false;

    for (int i = 0 ; i < nMaxAO ; i++) {
      gtwEncodePtr->iSetAO[i] = slaves[slaveIndex].AO[i];
    }

    gtwEncodePtr->iSetDO = slaves[slaveIndex].DigOut;
  }
